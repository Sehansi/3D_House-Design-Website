...
// Get recent public designs
router.get('/public/recent', async (req, res) => {
  try {
    const designs = await Design.find()
      .sort({ createdAt: -1 })
      .limit(6) // Get top 6 recent designs
      .populate('user', 'name') // Optionally get user details if you want to show creator
      .select('-modelData'); // Exclude heavy model data
